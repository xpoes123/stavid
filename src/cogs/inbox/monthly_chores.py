"""#monthly-chores → ChoreTemplate(recurrence='monthly')."""
...
CHANNEL = "monthly-chores"
RECURRENCE = "monthly"
log = logging.getLogger(__name__)


class MonthlyChoresInbox(commands.Cog):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        try:
            ...
            log.exception("monthly-chores inbox failed for message %s", message.id)
            await react_warn(message)


async def setup(bot: commands.Bot) -> None:
    await bot.add_cog(MonthlyChoresInbox(bot))
