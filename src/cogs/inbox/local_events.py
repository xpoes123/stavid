"""#local-events → OutingWishlistItem with category=\"activity\".

Same shape as #things-to-do but split into its own channel/cog so each can be
...
class LocalEventsInbox(commands.Cog):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        try:
            ...
            log.exception("local-events inbox failed for message %s", message.id)
            await react_warn(message)


async def setup(bot: commands.Bot) -> None:
    await bot.add_cog(LocalEventsInbox(bot))
